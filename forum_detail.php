...
if (isset($_SESSION['token'])) {
    ...
    $user_info_url = "http://143.198.218.9:8000/api/me"; // URL API untuk mendapatkan informasi pengguna
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($user_id) {
        ...
        if ($action === 'like' || $action === 'dislike') {
            $api_url = "http://143.198.218.9:8000/api/forums/$id/$action";
            ...
            if (!empty($comment_content)) {
                $api_url = "http://143.198.218.9:8000/api/forums/comment";
...
// Ambil data dari API
$api_url = 'http://143.198.218.9:8000/api/forums/' . $id; // URL API kamu
...
if (!empty($forum['image'])) {
    $forum['image'] = 'http://143.198.218.9:8000/storage/forum/' . $forum['image'];
}
?>
